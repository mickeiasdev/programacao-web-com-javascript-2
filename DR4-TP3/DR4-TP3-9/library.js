...
function tamanhoMaximo(item, num) {
    return item.length < num
      ? item
      : alert(`o item ${item} tem mais de ${num} caracteres`);
  }
  
  function tamanhoMinimo(item, num) {
    return item.length < num
      ? alert(`o item ${item} tem menos de ${num} caracteres`)
      : item;
  }
  ...
  function numeroMax(num, max = 15) {
    if (num > max) {
      alert(`O numero maximo permitido é ${max}`);
      return;
    }
    return num;
  }
  
  function numeroMin(num, min = 6) {
    if (num < min) {
      alert(`O numero minimo permitido é ${min}`);
      return;
    }
    return num;
  }
